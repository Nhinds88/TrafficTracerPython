...
Op2 = numpy.ones((5,5),numpy.uint8)
...
def process_video(log, ret, frame, entry, exited, areaThreshold, entry_limit, entry_line, exit_limit, exit_line, line1, line2, line3, line4, persons):
            ...
            if cy in range(int(entry_limit[0]), int(exit_limit[0])):
                for i in persons:
                    ...
                    if i.getState() == '1':
                        ...
                        if i.getDir() == 'down' and i.getY() > exit_limit:
                            i.setDone()
                        elif i.getDir() == 'up' and i.getY() < entry_limit:
                            i.getDone()
                    if i.timedOut():
    ...
    frame = cv2.polylines(frame, [line1], False, (255,0,0), thickness=3)
    frame = cv2.polylines(frame, [line2], False, (0,0,255), thickness=3)
